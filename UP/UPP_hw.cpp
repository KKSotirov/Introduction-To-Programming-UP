...
void initMatrix(char **matrix, size_t rows, size_t cols)
{
    for (size_t i = 0; i < rows; i++)
    {
        ...
        for (size_t j = 0; j < cols; j++)
        {
            ...
            {
                std::cin >> letter;
                if (isUppercaseLetter(letter) == true)
                {
                    matrix[i][j] = letter;
                    break;
                }
                else
                    std::cout << "Invalid input! Please enter an uppercase letter!" << std::endl;
            }
        }
        std::cout << std::endl;
    }
...
bool isUpperCaseWord(char *words)
{
    for (size_t i = 0; words[i] != '\0'; i++)
    {
        if (isUppercaseLetter(words[i]) == false)
            return false;
    }
    return true;
}
...
int main()
{
    ...
    for (size_t i = 0; i < numbs; i++)
    {
        ...
        while (true)
        {
            ...
            if (isUpperCaseWord(words[i]) == true)
                break;
            else
                std::cout << "Invalid input! Use only UPPERCASE letters! ";
        }
    }
